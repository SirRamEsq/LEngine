...
function camera.Update()
  if(camera.blockFollow) then
    pos = camera.CPPInterface:EntityGetPositionWorldInt(camera.parent)

    pos.x = (math.floor(pos.x/camera.w) * camera.w)
    pos.y = (math.floor(pos.y/camera.h) * camera.h)

    camera.myPositionComp:SetPositionLocalInt(pos);
  else
