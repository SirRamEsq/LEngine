...
GameRunningState::GameRunningState(GameStateManager *gsm) : GameState(gsm) {}
...
void GameRunningState::HandleEvent(const Event *event) {
  if (event->message == Event::MSG::KEYDOWN) {
    ...
    if (inputKey == "use") {
      countdown = -1;
    } else if (inputKey == "pause") {
      gameStateManager->PushState(
          std::make_unique<GamePauseState>(gameStateManager));
    }
...
void GameRunningState::Draw() { renderMan.Render(); }
