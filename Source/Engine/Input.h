...
/**
 * Handles input from SDL, sends events to StateManager and Entities inside the
 * state
 */
class InputManager {
 public:
   ...
   * Process all events from SDL, sends events specified by the key mapping to
   * the dispatcher.
   ...
   * These functions will process their simulated input during the HandleInput
   * function
   */
   ...
   */
  std::shared_ptr<KeyMapping> SetEventDispatcher(
      EventDispatcher *e, std::shared_ptr<KeyMapping> *mapping = NULL);
