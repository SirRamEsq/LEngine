...
provide serious funding for development time.
 ...
 A: Overwrite the following files:
    ...
    If you have a problem with a missing function/symbols, search for its name
in the code, there will likely be a comment about it.
...
ImGuiStyle::ImGuiStyle() {
  ...
  ChildWindowRounding =
      0.0f;  // Radius of child window corners rounding. Set to
...
static void SaveIniSettingsToDisk(const char *ini_filename) {
  ...
  for (int i = 0; i != g.Settings.Size; i++) {
    ...
    if (const char *p =
            strstr(name, "###"))  // Skip to the "###" marker if any.
...
static void AddDrawListToRenderList(ImVector<ImDrawList *> &out_render_list,
  ...
  // window)
  // If this assert triggers because you are drawing lots of stuff manually, you
  // can:
  // A) Add '#define ImDrawIdx unsigned int' in imconfig.h to set the index size
  // to 4 bytes. You'll need to handle the 4-bytes indices to your renderer.
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
  ...
  if (g.IO.MouseDown[button])
    if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
      return g.IO.MousePos -
             g.IO.MouseClickedPos[button];  // Assume we can only
                                            // get active with
                                            // left-mouse button
                                            // (at the moment).
...
void ImGui::BeginTooltip() { BeginTooltipEx(false); }
  ...
  if (g.SetNextWindowPosCond) {
    ...
    if (window_pos_set_by_api &&
        ImLengthSqr(g.SetNextWindowPosVal - ImVec2(-FLT_MAX, -FLT_MAX)) <
            0.001f) {
      window->SetWindowPosCenterWanted = true;  // May be processed on the next
                                                // frame if this is our first
                                                // frame
  ...
  if (first_begin_of_the_frame) {
    ...
    if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api) {
      ImRect rect_to_avoid(
          ...
          g.IO.MousePos.y +
              24);  // FIXME: Completely hard-coded. Perhaps center
    ...
    if (!window->Collapsed && !window->SkipItems)
      window->Scroll = ImMin(
          window->Scroll,
          ImMax(ImVec2(0.0f, 0.0f), window->SizeContents - window->SizeFull +
...
static void SetWindowPos(ImGuiWindow *window, const ImVec2 &pos,
  ...
  window->DC.CursorMaxPos +=
      (window->Pos -
       old_pos);  // And more importantly we need to adjust this so
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size,
  ...
  if (focus_requested || user_clicked || user_scrolled) {
    if (g.ActiveId != id) {
      ...
      edit_state.CurLenA =
          (int)(buf_end -
                buf);  // We can't get the result from ImFormatString()
...
bool ImGui::Combo(const char *label, int *current_item,
  ...
  RenderCollapseTriangle(
      ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y) + style.FramePadding,
      true);
  ...
  switch (edit_mode) {
    ...
    case ImGuiColorEditMode_HEX: {
      ...
      if (alpha)
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", i[0], i[1],
                       i[2]);
...
void ImGui::NewLine() {
  ...
  if (window->DC.CurrentLineHeight >
      0.0f)  // In the event that we are on a line
