...
CollisionBox::CollisionBox(int order, uint8_t flags, const Shape *shape,
...
bool CollisionBox::operator<(const CollisionBox &rhs) const {
  ...
  if ((mFlags & PRIMARY) == PRIMARY) {
    return true;
  }
  if ((rhs.mFlags & PRIMARY) == PRIMARY) {
    return false;
  }
...
void CollisionBox::Activate() { mActive = true; }

void CollisionBox::Deactivate() { mActive = false; }
