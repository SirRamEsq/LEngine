...
/*
The CPU will determine how many Particles to render based on its state
(STARTING, STOPPING, SUSTAINING, etc...)
and by how much time has passed since having been started
It will also insert 'new' particles (new particles being freshly seeded random
values) into the GPU buffer (is treated as a ring buffer)
Each Particle Creator will have two draw calls, one for the head of the buffer
...
// Particle Creator//
////////////////////
ParticleCreator::ParticleCreator(RenderManager *rm,
                                 const unsigned int &particleLife,
                                 const bool &useSprite)
    : RenderableObjectWorld(rm),
      dependencyRenderManager(rm),
      mParticleLifeSpan(particleLife),
      mUseSprite(useSprite),
      VerticiesPerParticle(4) {
...
void ParticleCreator::SetUsePoint(bool value) { mUsePoint = value; }
...
bool ParticleCreator::Start() {
  ...
  if (vbo == 0) {
    ...
    vboBufferSize =
        mMaxParticles * sizeof(ParticleVertexData) * VerticiesPerParticle;
  ...
  // The particle creator will begin generating particles, but this state
  // indicates that
...
void ParticleCreator::SetShape(const PARTICLE_SHAPE &shape) { mShape = shape; }
...
std::unique_ptr<RSC_GLShader> ParticleCreator::GenerateFragmentShader() {
  ...
  std::string particleShape =
      PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_SHAPE_SQUARE;
  switch (mShape) {
    case PARTICLE_CIRCLE:
      particleShape = PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_SHAPE_CIRCLE;
      break;
    case PARTICLE_RING:
      particleShape = PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_SHAPE_RING;
      break;
  }
  ...
  switch (mEffect) {
    case PARTICLE_FADE_IN:
      particleEffect = PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_EFFECT_FADE_IN;
      break;
    case PARTICLE_FADE_OUT:
      particleEffect = PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_EFFECT_FADE_OUT;
      break;
  }
  ...
  if (mScriptShaderCodeFragment == "") {
    scriptCodeFragment << PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_BEGIN
                       << particleShape << particleEffect
                       << PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_END;
  } else {
    scriptCodeFragment << mScriptShaderCodeFragment;
  }
  std::stringstream ss;
  ss << PARTICLE_SHADER_FRAGMENT_DECLARATIONS
     << PARTICLE_SHADER_FRAGMENT_MAIN_BEGIN;
  if (mSprite != NULL) {
    scriptCodeFragment << PARTICLE_SHADER_FRAGMENT_MAIN_SPRITE_END;
  }
  ss << scriptCodeFragment.str() << PARTICLE_SHADER_FRAGMENT_MAIN_END;

  std::unique_ptr<RSC_GLShader> shader(
      new RSC_GLShader(ss.str(), SHADER_FRAGMENT));
...
std::unique_ptr<RSC_GLShader> ParticleCreator::GenerateVertexShader() {
  std::stringstream ss;
  ss << PARTICLE_SHADER_VERTEX_DECLARATIONS;
  if (mUsePoint) {
  ...
  std::unique_ptr<RSC_GLShader> shader(
      new RSC_GLShader(ss.str(), SHADER_VERTEX));
...
void ParticleCreator::SetParticlesPerFrame(const float &particles) {
  /// \TODO bad things will happen if this is changed while the particle creator
  /// is running
  if (particles > 0) {
...
void ParticleCreator::SetColor(const float &rMin, const float &gMin,
...
void ParticleCreator::SetScalingX(const float &xscaleMin,
                                  const float &xscaleMax) {
  mXScalingMin = xscaleMin;
  mXScalingMax = xscaleMax;
}
void ParticleCreator::SetScalingY(const float &yscaleMin,
                                  const float &yscaleMax) {
  mYScalingMin = yscaleMin;
  mYScalingMax = yscaleMax;
}

void ParticleCreator::SetWarpQuads(bool value) { mWarpQuads = value; }

// Location and Size is in particles, not bytes or verticies
void ParticleCreator::WriteData(const unsigned int &writeLocation,
                                const unsigned int &writeSize) {
  if (writeSize == 0) {
    return;
  }
  bool ringLoop =
      false;  // whether there is one single contiguous memory buffer or two
  ...
  for (int i = 0; i < writeSize; i++) {
    ...
    // If using a point to move toward/away from, then store the point in the
    // acceleration vector
    if (mUsePoint) {
      ...
      acceleration =
          Vec2(mRandom.GenerateRandomFloatValue(mDefaultAccelerationMin.x,
    ...
    for (int vert = 0; vert < 4; vert++) {
      ...
      if (vert == 0) {
        offset.x = -1 * scaling.x;
        offset.y = -1 * scaling.y;
        tex.x = texCoordLeft;
        tex.y = texCoordTop;
      }
      if (vert == 1) {
        if (mWarpQuads) {
          position = Vec2(mRandom.GenerateRandomFloatValue(
                              mDefaultPositionMin.x, mDefaultPositionMax.x),
                          mRandom.GenerateRandomFloatValue(
                              mDefaultPositionMin.y, mDefaultPositionMax.y));
        }
        offset.x = 1 * scaling.x;
        offset.y = -1 * scaling.y;
        tex.x = texCoordRight;
...
void ParticleCreator::Stop() {
  if ((mState == PARTICLE_CREATOR_STOPPING) or
      (mState == PARTICLE_CREATOR_STOPPED)) {
    return;
  }
...
void ParticleCreator::Update() {
  if (mState == PARTICLE_CREATOR_STOPPED) {
    return;
  }
  ...
  // This is the number of particles that need updated this frame (starting from
  // the write buffer) to reach the new index
  int indexDifference = abs(indexAfter - indexBefore);

  if ((mState == PARTICLE_CREATOR_STARTING) or
  ...
  // Creator has stopped drawing new particles but still has remaining living
  // particles
  // Render particles starting at read Position, then increment read position by
  // mParticlesPerFrame
...
void ParticleCreator::Render(const RenderCamera *camera,
  ...
  /*
  If the state is starting or sustaining, we can just start at the the beginning
  of the buffer and render 'mParticlesToRender' amount of particles
  if the state is stopping, then we need to render starting at
  'mParticleBufferReadLocation' and continue on by 'mParticlesToRender' amount
  Also, if the active part of the buffer is split between the end and the start
  of the buffer (which it likely is) we'll need two draw calls
  */
...
ComponentParticle::ComponentParticle(EID id, ComponentPosition *pos,
                                     RenderManager *rm,
                                     ComponentParticleManager *manager)
...
ParticleCreator *ComponentParticle::AddParticleCreator(
    const unsigned int &creatorLife, const unsigned int &particleLife) {
  mParticleCreators.push_back(std::make_unique<ParticleCreator>(
      dependencyRenderManager, particleLife, false));
...
void ComponentParticle::DeleteParticleCreators() { mParticleCreators.clear(); }

//////////////////////////////
// Component Particle Manager//
//////////////////////////////
ComponentParticleManager::ComponentParticleManager(EventDispatcher *e)
...
void ComponentParticleManager::SetDependencies(RenderManager *rm,
                                               ComponentPositionManager *pos) {
  dependencyRenderManager = rm;
