...
struct ParticleVertexData {
  ...
  Vec2 acceleration;
  Vec4 color;
  Vec2 lifeTime;  // lifeTime.x is the timeStamp at which the particle was
                  // created, lifeTime.y is the timeStamp to be destroyed at
...
class ParticleCreator : public RenderableObjectWorld {
 ...
 public:
  ParticleCreator(RenderManager *rm, const unsigned int &particleLife,
  ...
  void SetColor(const float &rMin, const float &gMin, const float &bMin,
                const float &aMin, const float &rMax, const float &gMax,
  ...
  /// Changed parameters do not take effect till stopping and starting the
  /// creator
 ...
 private:
  void WriteData(const unsigned int &writeLocation,
  ...
  // Describes which ParticleVertexData to read/write from, not exactly which
  // byte;
...
class ComponentParticle : public BaseComponent {
 public:
  ComponentParticle(EID id, ComponentPosition *pos, RenderManager *rm,
  ...
  ParticleCreator *AddParticleCreator(const unsigned int &creatorLife,
...
class ComponentParticleManager
    : public BaseComponentManager_Impl<ComponentParticle> {
 public:
  ComponentParticleManager(EventDispatcher *e);
  ~ComponentParticleManager();

  std::unique_ptr<ComponentParticle> ConstructComponent(
...
extern const std::string
    PARTICLE_SHADER_FRAGMENT_MAIN_LUASTRING_EFFECT_FADE_OUT;
