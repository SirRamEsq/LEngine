...
GIDEnabled::GIDEnabled(GIDManager *man, GID first, GID last)
  ...
  if (firstGID > lastGID) {
    LOG_INFO(
        "[C++] GIDEnabled Constructor: lowest gid passed is greater than "
        "highest GID passed");
  }
...
GIDEnabled::GIDEnabled(GIDManager *man) : manager(man) {
  firstGID = 0;
  lastGID = 0;
}
...
GID GIDEnabled::GetFirstGID() const { return firstGID; }

GID GIDEnabled::GetLastGID() const { return lastGID; }
...
GIDManager::Range GIDManager::NewRange(const GID &first, const GID &last,
                                       GIDEnabled *item) {
  Range r(first, last);
  GIDItems[r] = item;
  return r;
