...
// So far, this only includes tile sets. backgrounds may later also be GID
// Enabled
// Tiled objects do not need to be GID enabled, they are all unique and don't
// need to be looked up with a Global ID

typedef unsigned long int GID;
class GIDManager;

// A GID is a global identifier that is used to look up data that is relevant to
// a certain resource
// A GID enabled class is a class that makes use of a range of GID values

// Once a GID enabled item is created, it's expected to not be deleted until the
// manager is
class GIDEnabled {
 public:
