...
double atan2Degree(double valy, double valx) {
  return ((atan2(valy, valx)) * 180) /
         LENGINE_DEF_PI;  // atan2(y,x); if 'y' is negative, output is negative
}

double ConvertToPositiveAngle(double angle) {
  angle = fmod(angle, 360.0f);  // Get value between 0 and 360
  while (angle <
         0.0f) {  // pretty sure this comparison is valid for doubles and floats
    angle += 360.0f;
  }
  return angle;
}

int ConvertInt(double val) { return floor(val + 0.5f); }
...
int RSC_Heightmap::DetermineHAngle() {
  /*int newAngle=0;
  int heightDiff=heightMapH[15]-heightMapH[0]; //Get the difference in between
  the last and first height values
  ...
  if (highestX == lowestX) {
    return 0;
  }  // If the highest is the same as the lowest, It's flat
  ...
  int heightDiff = highest - lowest;  // Get the difference in between the last
  // height value above zero and first height
  // value above zero
  int xDiff;
  if (Dir == true) {
    xDiff = highestX - lowestX;
  } else {
    xDiff = lowestX - highestX;
    heightDiff *= -1;
  }

  // Uses negative Angles                                            //(where up
  // is positive y)
  // newAngle= atan2Degree(heightDiff,xDiff); //atan2(y,x); if 'y' is negative,
  // output is negative
  // return newAngle;

  // Uses 360 Degrees
  newAngle = atan2Degree(
      heightDiff, xDiff);  // atan2(y,x); if 'y' is negative, output is negative
...
int RSC_Heightmap::DetermineHAngle(){
    ...
    if(highestX==lowestX){return 0;}//If the highest is the same as the lowest,
It's flat
    ...
    int heightDiff=highest-lowest; //Get the difference in between the last
height value above zero and first height value above zero
    int xDiff;
    if(Dir==true){
        xDiff= highestX-lowestX;
    }
                                                ...
                                                //(where up is positive y)
    newAngle= atan2(heightDiff,xDiff); //atan2(y,x); if 'y' is negative, output
is negative
    newAngle= newAngle * 180 / LENGINE_DEF_PI; //get angle in degrees
    return newAngle;
}*/
...
int RSC_Heightmap::DetermineVAngle() {
  /*int newAngle=0;
  int heightDiff=heightMapV[15]-heightMapV[0]; //Get the difference in between
  the last and first height values
                                              //(where up is positive y)
  newAngle= atan2(heightDiff,15); //atan2(y,x)
  newAngle= newAngle * 180 / LENGINE_DEF_PI; //get angle in degrees
...
RSC_Heightmap::RSC_Heightmap(int8_t hmHparam[16], int8_t hmVparam[16],
                             int angleHparam, int angleVparam) {
  SetHeightmapH(hmHparam);
  SetHeightmapV(hmVparam);
...
int RSC_Heightmap::GetHeightMapH(int index) { return heightMapH[index]; }

int RSC_Heightmap::GetHeightMapV(int index) { return heightMapV[index]; }
