...
void LAnimation::SetColorKey(int image, unsigned int r, unsigned int g,
                             unsigned int b) {
...
LAnimation::~LAnimation() { DeleteImages(); }

void LAnimation::DeleteImages() { images.clear(); }
...
void RSC_Sprite::SetColorKey(const std::string &aniName, unsigned int image,
...
void RSC_Sprite::SetOrigin(LOrigin o) { origin = o; }
...
LAnimation *RSC_Sprite::LoadAnimation(rapidxml::xml_node<> *animationNode) {
  ...
  for (imageNode = animationNode->first_node(); imageNode != 0;
    ...
    if (attribute != NULL) {
      rect.x = strtol(attribute->value(), NULL, 10);
    }

    attribute = imageNode->first_attribute("y");
    if (attribute != NULL) {
      rect.y = strtol(attribute->value(), NULL, 10);
    }

    attribute = imageNode->first_attribute("w");
    if (attribute != NULL) {
      rect.w = strtol(attribute->value(), NULL, 10);
    }
...
LAnimation *RSC_Sprite::LoadAnimationSequence(
    rapidxml::xml_node<> *animationNode) {
  using namespace rapidxml;
  ...
  animations.insert(std::make_pair(
      animationName,
      LAnimation((atof(speed.c_str())), LOAD_TAG_ANIMATION_SEQUENCE)));
  animation = &animations.find(animationName)->second;

  // Get parameters from xml node attributes
  attribute = animationNode->first_attribute("startX");
  if (attribute != NULL) {
    startRect.x = strtol(attribute->value(), NULL, 10);
  }

  attribute = animationNode->first_attribute("startY");
  if (attribute != NULL) {
    startRect.y = strtol(attribute->value(), NULL, 10);
  }

  attribute = animationNode->first_attribute("w");
  if (attribute != NULL) {
    startRect.w = strtol(attribute->value(), NULL, 10);
  }

  attribute = animationNode->first_attribute("h");
  if (attribute != NULL) {
    startRect.h = strtol(attribute->value(), NULL, 10);
  }

  attribute = animationNode->first_attribute("imgs");
  if (attribute != NULL) {
    imgs = strtol(attribute->value(), NULL, 10);
  }
...
std::unique_ptr<RSC_Sprite> RSC_Sprite::LoadResource(const std::string &fname) {
  ...
  try {
    ...
    if (sprite->LoadFromXML(data.get()->GetData(), data.get()->length) ==
        false) {
