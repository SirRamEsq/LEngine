...
RenderText::RenderText(RenderManager *rm, int xv, int yv, std::string t,
                       bool abso)
...
void RenderText::Render(const RenderCamera *camera,
  ...
  if (mTexture == NULL) {
    SDL_Surface *surface =
        TTF_RenderUTF8_Blended(defaultFont, text.c_str(), color);
    if (surface == NULL) {
  ...
  if (VBOID == 0) {
    ...
    // Texture coordinates
    vData[0].texCoord.s = 0;
    vData[0].texCoord.t = 0;
    vData[1].texCoord.s = 1;
    vData[1].texCoord.t = 0;
    vData[2].texCoord.s = 1;
    vData[2].texCoord.t = 1;
    vData[3].texCoord.s = 0;
    vData[3].texCoord.t = 1;
    // Vertex positions
    vData[0].position.x = 0;
    vData[0].position.y = 0;
    vData[1].position.x = w;
    vData[1].position.y = 0;
    vData[2].position.x = w;
    vData[2].position.y = h;
    vData[3].position.x = 0;
    vData[3].position.y = h;
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 4 * sizeof(GLuint), iData,
                 GL_DYNAMIC_DRAW);
    // Unbind buffers
    // glBindBuffer( GL_ARRAY_BUFFER, NULL ); glBindBuffer(
