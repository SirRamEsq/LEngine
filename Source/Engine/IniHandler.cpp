...
void IniHandler::ClearReadMap() { readMap.clear(); }

void IniHandler::ClearWriteMap() { writeMap.clear(); }
...
bool IniHandler::ReadFileToMap() {
  ...
  while (!iniRead.eof()) {
    ...
    // If the string contains no characters or only one blank character,
    // continue
    if (CstringSize < 2) {
      break;
    }
