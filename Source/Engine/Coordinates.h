...
class Coord2df {
 public:
  ...
  inline Coord2df operator+(float f) const { return Coord2df(x + f, y + f); }
  inline Coord2df operator-(float f) const { return Coord2df(x - f, y - f); }
  ...
  inline Coord2df Truncate() const { return Coord2df(trunc(x), trunc(y)); }

  /// Lua Interface Support Function
  Coord2df Add(const Coord2df &c) { return (*this) + c; }
  Coord2df Subtract(const Coord2df &c) { return (*this) - c; }
...
class Shape {
 public:
  ...
  inline void SetPos(const Coord2df pos) {
    x = pos.x;
    y = pos.y;
  }
...
class Rect : public Shape {
 public:
  ...
  inline void SetSize(Coord2df size) {
    w = size.x;
    h = size.y;
  }
