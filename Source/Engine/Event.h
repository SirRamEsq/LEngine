 ...
 public:
  ...
  class ExtraDataDefinition {
   public:
    ...
    // write a constructor that grabs the data structure you want to set to
    // extradata
    // store the datastructure in a member variable
    // assign the member variable to the event->extradata in the SetExtraData
    // function
    virtual void SetExtraData(Event *event) = 0;
    // Should write a static getter to convert the void pointer to the desired
    // type
  };
  ...
  Event(EID send, EID rec, MSG mes, const std::string &desc,
        ExtraDataDefinition *ed = NULL);
