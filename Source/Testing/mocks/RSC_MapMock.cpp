...
RSC_MapMock::RSC_MapMock(const std::string &mName, unsigned int w, unsigned int h)
    : gid(),
      mapName(mName),
      widthTiles(w),
      heightTiles(h),
...
unsigned int RSC_MapMock::GetWidthPixels() const { return widthTiles * tileWidth; }
unsigned int RSC_MapMock::GetHeightPixels() const { return heightTiles * tileHeight; }
