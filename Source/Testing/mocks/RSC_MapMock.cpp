...
RSC_MapMock::RSC_MapMock(const std::string& mName, int w, int h)
    : gid(), mapName(mName), widthTiles(w), heightTiles(h), layer(w, h, "LAYER", 0, &gid){
}

 RSC_MapMock::~RSC_MapMock(){
 }

EID RSC_MapMock::GetEIDFromName(const std::string& name)const{
    return 0;
}
...
int RSC_MapMock::GetWidthTiles() const{
    return widthTiles;
}
int RSC_MapMock::GetHeightTiles() const{
    return heightTiles;
}
int RSC_MapMock::GetWidthPixels() const{
    return widthTiles * tileWidth;
}
int RSC_MapMock::GetHeightPixels() const{
...
const TiledTileLayer* RSC_MapMock::GetTileLayerCollision(int x, int y, bool areTheseTileCoords) const{
