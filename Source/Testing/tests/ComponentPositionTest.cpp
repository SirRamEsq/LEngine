...
TEST_CASE("Test ComponentPosition and Manager", "[position]") {
  ...
  SECTION("Parents can be added and updated correctly") {
    ...
    REQUIRE(child->GetPositionWorld().x ==
            Approx(childX + parentX + parentXspd));
    REQUIRE(child->GetPositionWorld().y ==
            Approx(childY + parentY + parentYspd));
    // Local should be the same as its initial value, the child's position
    // relative to its parent hasn't changed
  ...
  SECTION("Ensure that coordinates can be translated between world and local") {
    ...
    auto translatedToLocal =
        child->TranslateWorldToLocal(child->GetPositionWorld());
    auto translatedToWorld =
        child->TranslateLocalToWorld(child->GetPositionLocal());
    ...
    REQUIRE(child->GetPositionLocal().x ==
            Approx(childWorldX - parent->GetPositionWorld().x));
    REQUIRE(child->GetPositionLocal().y ==
            Approx(childWorldY - parent->GetPositionWorld().y));
  }
